...
const collectionName = 'scheduleMatch';
...
router.post('/createMatch', auth.basicAuth, schema.matchSchedule, async (req, res) => {
    try {
        let data = req.body;
        if(data.teamId == data.opponentTeamId) {
            res.status(400).json({ status: 400, message: "Team and opponent team should not match!!!" })
        }
        data.teamId = req.body.teamId ? new ObjectId(req.body.teamId) : 0;
        data.score = req.body.score ? req.body.score : 0;
        data.opponentTeamScore = req.body.opponentTeamScore ? req.body.opponentTeamScore : 0;
        data.wicket = req.body.wicket ? req.body.wicket : 0;
        data.opponentTeamSicket = req.body.opponentTeamSicket ? req.body.opponentTeamSicket : 0;
